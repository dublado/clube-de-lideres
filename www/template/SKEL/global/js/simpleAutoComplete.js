...
(function($){
	$.fn.extend(
	{
		simpleAutoComplete: function( page, options, callback )
		{
			...
			thisElement.keyup(function( ev )
			{
				...
				var smartOptions = thisElement.val();
				...
				if( typeof(options) == "object" )
				{
					...
					getOptions.identifier = options.identifier; smartOptions = smartOptions + '/' + options.identifier;
				...
				if (key.match(/[a-zA-Z0-9_\- ]/) || kc == 8 || kc == 46)
				{
					...
					//alert(page+"/"+smartOptions);
					$.get(page+"/"+smartOptions, "", function(r)
					{
						$('div.' + classAC).remove();
	...
	{
	    $('#busca').simpleAutoComplete('../ajax/global/autocomplete',{
		autoCompleteClassName: 'autocomplete',
		selectedClassName: 'sel',
		attrCallBack: 'rel',
		identifier: 'produto'
